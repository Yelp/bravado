...
class Operation(object):
    ...
    :param operation_spec: operation specification in dict form
    """
    def __init__(self, swagger_spec, path_name, http_method, operation_spec):
    ...
    @classmethod
    def from_spec(cls, swagger_spec, path_name, http_method, operation_spec):
        """
        ...
        """
        op = cls(swagger_spec, path_name, http_method, operation_spec)
        op.build_params()
        return op

    @property
    def resolved_param_specs(self):
        """
        Returns param_specs for the current operation with immediate $refs
        resolved.

        :return: list of dicts representing parameter specs
        """
        specs = []
        for param_spec in self.operation_spec.get('parameters', []):
            resolved_param_spec = self.swagger_spec.resolve(param_spec)
            specs.append(resolved_param_spec)
        return specs

    def build_params(self):
        """
        Builds up the list of this operations parameters taking into account
        parameters that may be available at #/parameters.
        """
        self.params = {}
        for param_spec in self.resolved_param_specs:
